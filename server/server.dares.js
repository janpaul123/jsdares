			...
			if (error) throw error;
				...
				db.dares.insert([
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000002')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Moving some more";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>Again, move the robot to the green square in as few lines as you can.</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 16;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 48;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000003')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Multiple goals";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>This time you have to visit <strong>all three</strong> goals, in any order. Programmers always look for the fastest solution. Can you find a fast route?</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 20;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 48;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000004')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Manipulation";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>For this dare you just have to edit the numbers in the program. Move the mouse over a number and drag it to quickly change its value.</p> <p>You can get <strong>extra points</strong> by visiting all goals!</p>";\nconfig.dare.RobotGoal.optionalGoals = 2;\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 11;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 32;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000006')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Zig-zag";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>For this you need to write your <strong>own</strong> function. You can try writing a program without a function, but note that you can only use <strong>20 lines</strong> (not counting empty lines and lines with only <var>}</var>).</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 20;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 32;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.function[0],robot.drive,robot.turnLeft,robot.turnRight";\nconfig.outputs.info.scope = false;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000007')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "ForwardRight";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>Sometimes you want to use the same commands, but only slightly different every time. In this dare, you want to move forward and then right, but with a different distance every time.</p><p>For this you can use an <strong>argument</strong> in the function. After the function name you give a name for the argument, and the argument then <strong>contains</strong> the number you put in when calling the function. You can then use this name when calling the commands in the function.</p><p>We have created an example for you, try to see what happens when you <a href=\\"#arrow-step\\">step</a> through it.</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 20;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 32;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.function[0],robot.drive,robot.turnLeft,robot.turnRight";\nconfig.outputs.info.scope = false;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000008')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "More functions";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>For this one you probably need to make one (or more) functions, since you can use no more than <strong>17 lines</strong> of code.</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 17;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 32;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.function[0],robot.drive,robot.turnLeft,robot.turnRight";\nconfig.outputs.info.scope = false;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000010')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Zoo";\nconfig.dare.type = "ImageMatch";\nconfig.dare.description = "<p>When you can draw one animal, you can draw a zoo, using a function. We have again provided you with a program that draws the head of some animals. If you find this dare too hard, you can leave it for now and try it again later.</p>";\nconfig.dare.ImageMatch.speed = 200;\nconfig.dare.ImageMatch.minPercentage = 97;\nconfig.dare.maxLines = 10;\nconfig.dare.lineReward = 10;\nconfig.outputs.canvas.enabled = true;\nconfig.outputs.canvas.size = 512;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.arithmetic.numbers[1],jsmm.function[0],canvas.getContext,context.fillRect,context.fillStyle";\nconfig.outputs.info.scope = false;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000103')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "More math";\nconfig.dare.type = "ConsoleMatch";\nconfig.dare.description = "<p>For this dare you have to use all the basic math operators: addition, subtraction, multiplication, and division. See the <a href=\\"#arrow-tab-info\\">info tab</a> to find out which is which. We already added the line for multiplication. For bonus points, you can try to figure out what the last call to <var>calculate</var> should be.</p>";\nconfig.dare.ConsoleMatch.speed = 200;\nconfig.dare.ConsoleMatch.minPercentage = 80;\nconfig.dare.maxLines = 13;\nconfig.dare.lineReward = 10;\nconfig.outputs.console.enabled = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.number,jsmm.string,jsmm.arithmetic.numbers,jsmm.arithmetic.strings,jsmm.function[0],jsmm.function[1],console.log,console.clear,console.setColor";\nconfig.outputs.info.scope = false;\n',
							original: 'function calculate(a, b) {\n  console.log("a + b = " + (a+b));\n  console.log("a - b = " + (a-b));\n  console.log("a * b = " + (a*b));\n  console.log("a / b = " + (a/b));\n  console.log("");\n}\n\ncalculate(8, 4);\ncalculate(10, 20);\ncalculate(0.5, 0.75);\ncalculate(-500, 500);\ncalculate(5, 4);',
							editor: {text: 'function calculate(a, b) {\n  console.log("a * b = " + (a*b));\n}\n\ncalculate(8, 4);\ncalculate(10, 20);\ncalculate(0.5, 0.75);\ncalculate(-500, 500);\n'}
						}}
					);
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000104')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Years ago";\nconfig.dare.type = "ConsoleMatch";\nconfig.dare.description = "<p>Using subtraction you can now calculate how many years before 2012 the famous people were born.</p>";\nconfig.dare.ConsoleMatch.speed = 200;\nconfig.dare.ConsoleMatch.minPercentage = 97;\nconfig.dare.maxLines = 22;\nconfig.dare.lineReward = 10;\nconfig.outputs.console.enabled = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.number,jsmm.string,jsmm.arithmetic.numbers,jsmm.arithmetic.strings,jsmm.function[0],jsmm.function[1],console.log,console.clear,console.setColor";\nconfig.outputs.info.scope = false;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000106')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Counting";\nconfig.dare.type = "ConsoleMatch";\nconfig.dare.description = "<p>The provided program prints the powers of 2 to the console. It uses a <strong>variable</strong> named <var>counter</var>, and a <strong>while loop</strong>. Try to figure out how it works by using the <a href=\\"#arrow-step\\">step button</a> and <a href=\\"#arrow-tab-info\\">info tab</a>. Then modify the program so that instead it prints the numbers from 1 to 20.</p><p>";\nconfig.dare.ConsoleMatch.speed = 200;\nconfig.dare.ConsoleMatch.minPercentage = 97;\nconfig.dare.maxLines = 5;\nconfig.dare.lineReward = 10;\nconfig.outputs.console.enabled = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.number,jsmm.string,jsmm.boolean,jsmm.var,jsmm.assignment,jsmm.arithmetic.numbers,jsmm.arithmetic.strings,jsmm.logic.equality,jsmm.logic.comparison,jsmm.logic.inversion,jsmm.logic.booleans,jsmm.while,jsmm.function[0],jsmm.function[1],console.log,console.clear,console.setColor";\nconfig.outputs.info.scope = true;\n',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000107')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "Spiraling out of control";\nconfig.dare.type = "RobotGoal";\nconfig.dare.description = "<p>The robot has to follow a certain pattern, as you can see in the program. However, the current program is a bit long and repetitive. Make it shorter by using a counter and a while loop.</p>";\nconfig.dare.RobotGoal.goalReward = 50;\nconfig.dare.maxLines = 7;\nconfig.dare.lineReward = 10;\nconfig.dare.RobotGoal.previewBlockSize = 32;\n\nconfig.outputs.robot.enabled = true;\nconfig.outputs.robot.readOnly = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.number,jsmm.string,jsmm.boolean,jsmm.var,jsmm.assignment,jsmm.arithmetic.numbers,jsmm.arithmetic.strings,jsmm.logic.equality,jsmm.logic.comparison,jsmm.logic.inversion,jsmm.logic.booleans,jsmm.while,jsmm.function[0],jsmm.function[1],robot.drive,robot.turnLeft,robot.turnRight";\nconfig.outputs.info.scope = true;\n',
							original: 'var counter = 1;\nwhile(counter <= 7) {\n  robot.drive(counter);\n  robot.turnLeft();\n  counter++;\n}',
					...
					db.dares.update({_id: new db.ObjectID('300000000000000000000109')},
						{ $set: {
							userId: user._id,
							configProgram: 'config.dare.name = "More information";\nconfig.dare.type = "ConsoleMatch";\nconfig.dare.description = "<p>Extend the table of famous people to include the year of death for those who have passed away. For those still alive, hide this line entirely.</p>";\nconfig.dare.ConsoleMatch.speed = 50;\nconfig.dare.ConsoleMatch.minPercentage = 97;\nconfig.dare.maxLines = 22;\nconfig.dare.lineReward = 10;\nconfig.outputs.console.enabled = true;\nconfig.outputs.info.enabled = true;\nconfig.outputs.info.commands = "jsmm.number,jsmm.string,jsmm.boolean,jsmm.var,jsmm.assignment,jsmm.arithmetic.numbers,jsmm.arithmetic.strings,jsmm.logic.equality,jsmm.logic.comparison,jsmm.logic.inversion,jsmm.logic.booleans,jsmm.if,jsmm.else,jsmm.while,jsmm.function[0],jsmm.function[1],console.log,console.clear,console.setColor";\nconfig.outputs.info.scope = true;\n',
