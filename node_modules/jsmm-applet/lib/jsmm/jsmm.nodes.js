...
module.exports = function(jsmm) {
	var addCommonNodeMethods = function(type, children, topNode, node) {
		...
		node.makeNodeIds = function() {
			this.id = this.tree.getNewId();
			this.tree.nodes[this.id] = this;
			var children = this.getChildren();
			for (var i=0; i<children.length; i++) {
				children[i].makeNodeIds();
			}
		};
	...
	jsmm.nodes.Program.prototype = addCommonNodeMethods('Program', {statementList: true}, false, {
		init: function() {
			// first give functions and their children node IDs, after that the main code
			var i, statement;
			for (i=0; i<this.statementList.statements.length; i++) {
				statement = this.statementList.statements[i];
				if (statement.type === 'FunctionDeclaration') {
					statement.makeNodeIds();
				}
			}
			for (i=0; i<this.statementList.statements.length; i++) {
				statement = this.statementList.statements[i];
			...
			this.id = this.tree.getNewId();
			this.tree.nodes[this.id] = this;
			this.statementList.id = this.tree.getNewId();
			this.tree.nodes[this.statementList.id] = this.statementList;
		},
		getCode: function() {
