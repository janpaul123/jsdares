...
module.exports = function(jsmm) {
	...
	jsmm.nodes.PostfixStatement.prototype.runFunc = function(context, variable, symbol) {
		...
		if (typeof value !== 'number') {
			...
			context.newStep([new jsmm.msg.Inline(this.id, '<var>' + this.identifier.getCode() + '</var> = <var>' + jsmm.stringify(value) + '</var>')]);
		}
	};
	...
	var runBinaryExpression = function(context, node, value1, symbol, value2) {
		if ((symbol === '+' || symbol === '+=') && (typeof value1 === 'string' || typeof value2 === 'string')) {
			context.addCommand(node, 'jsmm.+s');
		} else if (['+', '-', '*', '/', '%'].indexOf(symbol) >= 0) {
			context.addCommand(node, 'jsmm.+');
		} else if (['+=', '-=', '*=', '/=', '%='].indexOf(symbol) >= 0) {
			context.addCommand(node, 'jsmm.+=');
		} else if (['>', '>=', '<', '<='].indexOf(symbol) >= 0) {
			context.addCommand(node, 'jsmm.>');
	...
	jsmm.nodes.FunctionCall.prototype.runFunc = function(context, func, args) {
		...
		if (retVal === null) {
			...
			context.newStep([new jsmm.msg.Inline(this.id, '<var>' + this.identifier.getCode() + '(' + msgFuncArgs.join(', ') + ')' + '</var> = <var>' + jsmm.stringify(retVal) + '</var>')]);
		}
