...
.editor-highlighting, .editor-highlighting textarea {
	cursor: crosshair;
}
...
/// TOOLBAR ///
.editor-toolbar {
	@toolbar-bubble-arrow-size: @box-arrow-size;
	@toolbar-bubble-color: lighten(@editor-box-background-color, 5%);
	.editor-toolbar-step-bubble {
		.code-foreground();
		position: absolute;
		left: -@toolbar-bubble-arrow-size;
		top: 0px;
		background-color: @toolbar-bubble-color;
		padding: 5px;
		z-index: 1000;
		.border-radius(4px);
		...
		.editor-toolbar-step-bubble-arrow {
			#popoverArrow > .left(@toolbar-bubble-arrow-size, @toolbar-bubble-color);
			position: absolute;
			width: 0;
			height: 0;
			margin-right: -@toolbar-bubble-arrow-size;
			right: 0px;
		}
	...
	.editor-toolbar-run-bar {
		@transition: top 0.2s ease-in-out, margin-left 0.2s ease-in-out;
		.transition(@transition);
		top: 0px;

		&.editor-toolbar-run-disabled {
			top: 50px;
			margin-left: -32px;
		}
	...
	@editor-toolbar-run-step-bar-margin: -170px/2 + 10px/2 + 2;
	.editor-toolbar-run-step-bar-container {
		...
		&.editor-toolbar-run-step-bar-error {
			...
			width: 170px + 25px;
			margin-left: @editor-toolbar-run-step-bar-margin - 25px/2 - 1;
		}
	}
}