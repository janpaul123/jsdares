...
module.exports = (jsmm) ->
  ...
  jsmm.test.runAll = ->
    ...
    failed = 0
    name = undefined
    for name of jsmm.test.tests.succeed
      failed++  unless jsmm.test.runTest(name.replace(/_/g, " "), jsmm.test.tests.succeed[name], true, true)
    for name of jsmm.test.tests.fail_threeway
      failed++  unless jsmm.test.runTest(name.replace(/_/g, " "), jsmm.test.tests.fail_threeway[name], true, false)
    for name of jsmm.test.tests.fail_twoway
      failed++  unless jsmm.test.runTest(name.replace(/_/g, " "), jsmm.test.tests.fail_twoway[name], false, false)
    if failed <= 0
  ...
  jsmm.test.runTest = (name, code, threeway, succeed) ->
    ...
    if threeway and not jsmm.test.compareErrors(errorRaw, errorSafe, succeed)
    ...
    # return false;
    # }
    if threeway and consoleRaw.result isnt consoleSafe.result
    ...
    # return false;
    # }
    if threeway and succeed isnt (errorRaw is null)
      jsmm.test.printError1 name, "errorRaw", errorRaw, code
      return false
    if succeed isnt (errorSafe is null)
  ...
  jsmm.test.compareErrors = (error1, error2) ->
    if error1 is null and error2 is null
      true
    else if error1 is null or error2 is null
      false
    
    # } else if (error1.orig !== null || error2.orig !== null) {
    # 	return false;
    else
      error1.getHTML() is error2.getHTML()
