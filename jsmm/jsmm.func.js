...
module.exports = function(jsmm) {
	...
	jsmm.func.postfix = function(el, variable, symbol) {
		if (typeof variable.value !== 'number') {
			throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(variable.str) + ' is not a number'; });
		} else {
			if (symbol === '++') {
				variable.value++;
			} else {
	...
	jsmm.func.binary = function(el, expression1, symbol, expression2) {
		...
		if (expression1.value === undefined) {
			throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression1.str) + ' is ' + f('undefined'); });
		} else if (expression2.value === undefined) {
			throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression2.str) + ' is ' + f('undefined'); });
		} else if (expression1.value === null) {
			throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression1.str) + ' is ' + f('null'); });
		} else if (expression2.value === null) {
			throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression2.str) + ' is ' + f('null'); });
		} else if (['-', '*', '/', '%', '-=', '*=', '/=', '%=', '>', '>=', '<', '<='].indexOf(symbol) >= 0) {
			if (typeof expression1.value !== 'number' || !isFinite(expression1.value)) {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression1.str) + ' is not a number'; });
			} else if (typeof expression2.value !== 'number' || !isFinite(expression2.value)) {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression2.str) + ' is not a number'; });
			...
			if (['number', 'string'].indexOf(typeof expression1.value) < 0) {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression1.str) + ' is not a number or string'; });
			} else if (['number', 'string'].indexOf(typeof expression2.value) < 0) {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression2.str) + ' is not a number or string'; });
			}
		} else if (['&&', '||'].indexOf(symbol) >= 0) {
			if (typeof expression1.value !== 'boolean') {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression1.str) + ' is not a boolean'; });
			} else if (typeof expression2.value !== 'boolean') {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression2.str) + ' is not a boolean'; });
			}
		}
	...
	jsmm.func.unary = function(el, symbol, expression) {
		...
		if (symbol === '!') {
			if (typeof expression.value !== 'boolean') {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression.str) + ' is not a boolean'; });
			} else {
			...
			if (typeof expression.value !== 'number') {
				throw new jsmm.msg.Error(el, function(f){ return f(symbol) + ' not possible since ' + f(expression.str) + ' is not a number'; });
			} else {
	...
	jsmm.func.funcCall = function(el, func, args) {
		...
		return jsmm.func.funcWrapResult(el, func, appFunc.apply(func.parent || null, funcArgs));
	};
