...
module.exports = function(jsmm) {
	var getValue = function(context, node, expression) {
		...
		if (value === undefined) {
			throw new jsmm.msg.Error(node.id, node.tree, '<var>' + node.getCode() + '</var> is <var>undefined</var>');
		} else if (value === null) {
			throw new jsmm.msg.Error(node.id, node.tree, '<var>' + node.getCode() + '</var> is <var>null</var>');
		} else if (typeof value === 'number' && !isFinite(value)) {
			throw new jsmm.msg.Error(node.id, node.tree, '<var>' + node.getCode() + '</var> is not a valid number');
	...
	var setVariable = function(context, node, variableNode, variable, value) {
		if (typeof variable === 'object' && variable.type === 'newArrayValue') {
			throw new jsmm.msg.Error(node.id, node.tree, '<var>' + variableNode.getCode() + '</var> is <var>undefined</var>');
		} else if (typeof variable !== 'object' || ['variable', 'local'].indexOf(variable.type) < 0) {
			throw new jsmm.msg.Error(node.id, node.tree, 'Cannot assign <var>' + jsmm.stringify(value) + '</var> to <var>' + variableNode.getCode() + '</var>');
			...
			try {
				...
				if (typeof error === 'string') {
					throw new jsmm.msg.Error(node.id, node.tree, error);
				} else {
					throw error;
				}
			}
		} else if (typeof variable.value === 'object' && variable.value.type === 'functionPointer') {
			throw new jsmm.msg.Error(node.id, node.tree, 'Cannot assign a new value to function <var>' + variable.value.name + '</var>');
		} else {
	...
	jsmm.nodes.PostfixStatement.prototype.runFunc = function(context, variable, symbol) {
		...
		if (typeof value !== 'number') {
			throw new jsmm.msg.Error(this.id, this.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value) + '</var> is not a number');
		} else {
			if (symbol === '++') {
				value++;
			} else {
			...
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, '<var>' + this.identifier.getCode() + '</var> = <var>' + jsmm.stringify(value) + '</var>'));
		}
	};
	...
	var runBinaryExpression = function(context, node, value1, symbol, value2) {
		...
		if (['-', '*', '/', '%', '-=', '*=', '/=', '%=', '>', '>=', '<', '<='].indexOf(symbol) >= 0) {
			if (typeof value1 !== 'number' || !isFinite(value1)) {
				throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value1) + '</var> is not a number');
			} else if (typeof value2 !== 'number' || !isFinite(value2)) {
				throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value2) + '</var> is not a number');
			} else if (['/', '/=', '%', '%='].indexOf(symbol) >= 0 && value2 === 0) {
				throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since it is a division by zero');
			}
		} else if (['+', '+='].indexOf(symbol) >= 0) {
			if ([typeof value1, typeof value2].indexOf('string') >= 0) {
				if (['number', 'boolean', 'string'].indexOf(typeof value1) < 0) {
					throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value1) + '</var> is not a number, string, or boolean');
				} else if (['number', 'boolean', 'string'].indexOf(typeof value2) < 0) {
					throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value2) + '</var> is not a number, string, or boolean');
				}
			} else {
				if (typeof value1 !== 'number') {
					throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value1) + '</var> is not a number or string');
				} else if (typeof value2 !== 'number') {
					throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value2) + '</var> is not a number or string');
				}
			}
		} else if (['&&', '||'].indexOf(symbol) >= 0) {
			if (typeof value1 !== 'boolean') {
				throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value1) + '</var> is not a boolean');
			} else if (typeof value2 !== 'boolean') {
				throw new jsmm.msg.Error(node.id, node.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value2) + '</var> is not a boolean');
			}
		} else if (['==', '!='].indexOf(symbol) >= 0) {
	...
	jsmm.nodes.VarItem.prototype.runFunc = function(context, name) {
		...
		if (this.assignment === null) {
			...
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, '<var>' + this.name + '</var> = <var>undefined</var>'));
		}
	};
	...
	jsmm.nodes.BinaryExpression.prototype.runFunc = function(context, expression1, symbol, expression2) {
		...
		context.pushStep(new jsmm.msg.Inline(this.id, this.tree, '<var>' + jsmm.stringify(value1) + '</var> ' + symbol + ' <var>' + jsmm.stringify(value2) + '</var> = <var>' + jsmm.stringify(result) + '</var>'));
		return result;
	};
	...
	jsmm.nodes.UnaryExpression.prototype.runFunc = function(context, symbol, expression) {
		...
		if (symbol === '!') {
			...
			if (typeof value !== 'boolean') {
				throw new jsmm.msg.Error(this.id, this.tree, '<var>' + symbol + '</var> not possible since <var>' + jsmm.stringify(value) + '</var> is not a boolean');
			} else {
				result = !value;
			}
		} else {
		...
		if (symbol === '!' || this.expression.type !== 'NumberLiteral') {
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, '<var>' + symbol + jsmm.stringify(value) + '</var> = <var>' + jsmm.stringify(result) + '</var>'));
		}
		return result;
	};
	...
	jsmm.nodes.NameIdentifier.prototype.runFunc = function(context, name) {
		...
		if (val === undefined) {
			throw new jsmm.msg.Error(this.id, this.tree, 'Variable <var>' + name + '</var> could not be found');
		} else {
			return val;
		}
	};
	...
	jsmm.nodes.ObjectIdentifier.prototype.runFunc = function(context, identifier, property) {
		...
		if (typeof identifierValue !== 'object' || ['object', 'array'].indexOf(identifierValue.type) < 0) {
			throw new jsmm.msg.Error(this.id, this.tree, 'Variable <var>' + this.identifier.getCode() + '</var> is not an object</var>');
		} else if (identifierValue.properties[property] === undefined) {
			throw new jsmm.msg.Error(this.id, this.tree, 'Variable <var>' + this.identifier.getCode() + '</var> does not have property <var>' + property + '</var>');
		} else {
	...
	jsmm.nodes.ArrayIdentifier.prototype.runFunc = function(context, identifier, expression) {
		...
		if (typeof identifierValue !== 'object' || identifierValue.type !== 'array') {
			throw new jsmm.msg.Error(this.id, this.tree, 'Variable <var>' + this.identifier.getCode() + '</var> is not an array');
		} else if (typeof expressionValue !== 'number' && expressionValue % 1 !== 0) {
			throw new jsmm.msg.Error(this.id, this.tree, 'Index <var>' + this.expression.getCode() + '</var> is not an integer');
		} else {
	...
	jsmm.nodes.FunctionCall.prototype.runFunc = function(context, funcObject, args) {
		...
		if (typeof funcValue === 'object' && funcValue.type === 'function') {
			...
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'calling <var>' + this.identifier.getCode() + '(' + msgFuncArgs.join(', ') + ')' + '</var>'));
			var func = context.scope.getFunction(funcValue.name);
			if (func === undefined) throw new jsmm.msg.Error(this.id, this.tree, 'Function <var>' + funcValue.name + '</var> could not be found');
			retVal = func.call(null, context, funcArgs);
		} else {
			throw new jsmm.msg.Error(this.id, this.tree, 'Variable <var>' + this.identifier.getCode() + '</var> is not a function');
		}
		context.leaveCall();
		...
		if (retVal !== undefined) {
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, '<var>' + this.identifier.getCode() + '(' + msgFuncArgs.join(', ') + ')' + '</var> = <var>' + jsmm.stringify(retVal) + '</var>'));
		} else {
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'called <var>' + this.identifier.getCode() + '(' + msgFuncArgs.join(', ') + ')'));
		}
	...
	jsmm.nodes.ForBlock.prototype.runFunc = function(context, expression) {
		...
		var value = getValue(context, this.expression, expression);
		if (typeof value !== 'boolean') {
			throw new jsmm.msg.Error(this.id, this.tree, '<var>' + type + '</var> is not possible since <var>' + jsmm.stringify(value) + '</var> is not a boolean');
		} else {
			return value;
		}
	};
	...
	jsmm.nodes.FunctionDeclaration.prototype.runFuncDecl = function(context, name, func) {
		...
		if (context.scope.vars[name] !== undefined) {
			if (typeof context.scope.vars[name] === 'object' && ['function', 'functionPointer'].indexOf(context.scope.vars[name].type) >= 0) {
				throw new jsmm.msg.Error(this.id, this.tree, 'Function <var>' + name + '</var> cannot be declared since there already is a function with that name');
			} else {
				throw new jsmm.msg.Error(this.id, this.tree, 'Function <var>' + name + '</var> cannot be declared since there already is a variable with that name');
			}
		} else {
			context.scope.declareFunction(name, func);
			context.addAssignment(this, name);
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'declaring <var>' + this.name + this.getArgList() + '</var>', 'blockLoc'));
		}
	};
	...
	jsmm.nodes.FunctionDeclaration.prototype.runFuncEnter = function(context, args) {
		if (args.length < this.nameArgs.length) {
			...
			throw new jsmm.msg.Error(context.leaveCall().id, this.tree, 'Function expects <var>' + this.nameArgs.length + '</var> arguments, but ' + but);
		}

		var scopeVars = {}, msgFuncArgs = [];
		for (var i=0; i<this.nameArgs.length; i++) {
			if (args[i] === undefined) {
				throw new jsmm.msg.Error(context.leaveCall().id, this.tree, 'Argument <var>' + this.nameArgs[i] + '</var> is <var>undefined</var>');
			} else if (args[i] === null) {
				throw new jsmm.msg.Error(context.leaveCall().id, this.tree, 'Argument <var>' + this.nameArgs[i] + '</var> is <var>null</var>');
			} else {
		...
		context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'entering <var>' + fullName + '</var>'));
	...
	jsmm.nodes.ReturnStatement.prototype.runFunc = function(context, expression) {
		...
		if (!context.inFunction()) {
			throw new jsmm.msg.Error(this.id, this.tree, 'Cannot return if not inside a function');
		}

		var retVal;
		if (this.expression !== undefined && expression !== undefined) {
			retVal = getValue(context, this.expression, expression);
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'returning <var>' + jsmm.stringify(retVal) + '</var>'));
		} else {
			context.pushStep(new jsmm.msg.Inline(this.id, this.tree, 'returning nothing'));
		}
