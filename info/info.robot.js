...
module.exports = function(info) {
	info.tables.push({
		html: '<p><span class="info-table-output"><i class="icon-th icon-white"></i> robot</span></p><p>The robot tab provides a simple simulation of an actual robot, with commands to drive and turn. Additionally, walls can be placed in the environment, which can be detected by the robot. Squares can be coloured green, indicating a goal for the robot to go to, and these can also be detected by the robot. Note that when walls and squares are placed, the movement of the robot is restricted so that it can only drive on the grid.</p>',
		list: [
			{
