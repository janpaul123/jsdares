...
module.exports = function(output) {
	...
	output.Home.prototype = {
		init: function(ui, $div) {
			...
			this.$content.append('<p>This works best on the latest versions of <strong>Chrome</strong>, <strong>Firefox</strong>, and <strong>Safari</strong>. Please send any bugs, suggestions, and other comments to <a href="mailto:jsdare@janpaulposma.nl">jsdare@janpaulposma.nl</a>. <strong>Thanks!</strong></p>');

			this.$content.append('<h3>Principles</h3><p>There are a few principles underlying this project. First, I believe that building <strong>games</strong> is one of the most motivating things children can do to learn programming. For us it is not directly the goal, however, but merely a <strong>tool</strong>. It allows us to seamlessly <strong>connect</strong> to other fields, such as maths and physics, in a context that is <strong>meaningful</strong> for children. To teach the basics, an interesting and engaging environment can be by simulating <strong>robots</strong>, which can also connect with actual robot classes being given at some schools.</p> <p>Also important is that they learn a <strong>real skill</strong>. All programs written with the canvas can be used <strong>outside</strong> of this environment, as it is based on a standard API. On the other hand, the language used here, which I call <strong>js--</strong>, is a somewhat <strong>restricted</strong> version of JavaScript, to prevent them from feeling overwhelmed, and to avoid some bad parts of the language.</p> <p>Another real skill is being able to <strong>search</strong> for documentation and examples online, which is why only a concise command reference is given. Furthermore, children and teachers should be <strong>in control</strong>, and not dependent on a set curriculum, which means that they should be able to design dares themselves and <strong>share</strong> them with peers.</p> <p>And all this in a way that lets them truly <strong>experience</strong> the deep connection between the code and its meaning. This means they must have tools to <strong>explore</strong> the language; their programs; the computational space. Computers and programming tools change, but an accurate <strong>intuition</strong> of how computations behave is useful for the rest of their lives.</p>');
